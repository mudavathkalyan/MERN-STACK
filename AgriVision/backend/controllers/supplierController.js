...
const createSupplierProduct = asyncHandler(async (req, res) => {
    ...
    if (name === '' || address === '') {
        res.status(400);
        throw new Error('No Product Items');
    } else {
        // OpenCage Geocoding API options
        const apiKey = '5d823c653fa54b528b8ceb28218a5871'; // Replace with your actual OpenCage API key
        const geoCoderUrl = `https://api.opencagedata.com/geocode/v1/json?q=${encodeURIComponent(address)}&key=${apiKey}`;

        try {
            const response = await axios.get(geoCoderUrl);
            const geoResults = response.data.results;
            ...
            const latAndLong = geoResults[0].geometry;
